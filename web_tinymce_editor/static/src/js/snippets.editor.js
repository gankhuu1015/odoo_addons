odoo.define("web_tinymce_editor.snippets.editor", function (require) {
  "use strict";
  ...
  const MassMailingSnippetsMenu = snippetsEditor.SnippetsMenu.extend({
    ...
    custom_events: _.extend(
      {},
      snippetsEditor.SnippetsMenu.prototype.custom_events,
      {
    ...
    optionsTabStructure: [["design-options", _lt("Design Options")]],
    ...
    _insertDropzone: function ($hook) {
      ...
      $dropzone.attr(
        "data-editor-message",
        $hookParent.attr("data-editor-message")
      );
      $dropzone.attr(
    ...
    _updateRightPanelContent: function ({ content, tab }) {
      ...
      this.$(".o_we_customize_design_btn").toggleClass(
        "active",
    ...
    _onDropZoneOut: function () {
      ...
      if (
        $oEditable.find(".oe_drop_zone.oe_insert:not(.oe_vertical):only-child")
          .length
      ) {
        $oEditable[0].style.setProperty(
          "background-color",
          "transparent",
          "important"
        );
